...
const StarIcon = () => (
  <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
    <path d="M9 0L11.0206 6.9794L18 9L11.0206 11.0206L9 18L6.9794 11.0206L0 9L6.9794 6.9794L9 0Z" fill="#141414" />
  </svg>
);
    ...
    <>
      <section className="relative bg-white pt-[80px] pb-[60px]">
        <div className="max-w-[1200px] mx-auto px-6 flex flex-col items-center text-center">
          <span className="text-[14px] font-bold text-[#0096ff] mb-4">#1 AI Cover Letter Generator for GenZ</span>
          <h1 className="text-[32px] md:text-[56px] font-bold text-[#1a1b1f] leading-[1.1] tracking-[-0.02em] max-w-[900px] mb-6">
        ...
        <div className="mt-16 max-w-[1200px] mx-auto px-6">
          <p className="text-[14px] font-bold text-[#666666] uppercase tracking-[0.1em] mb-8 text-center opacity-60">Trusted around the world</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full mb-[48px]">
            {[{ icon: "🔍", title: "Step 1:\nFind a job post", desc: "Start searching for jobs anywhere you want and paste the job description in the box in JobExCV." }, { icon: "📄", title: "Step 2:\nPrepare the resume", desc: "Use JobExCV to prepare a tailored resume for the job in one click." }, { icon: "✉️", title: "Step 3:\nClick ... there you go", desc: "One click, and your cover letter is ready. Feel free to manually edit it if needed." }].map((step, i) => (
              <div key={i} className="flex flex-col items-center text-center p-4">
                ...
                <p className="text-[16px] text-[#666666] leading-[1.6]">{step.desc}</p>
              </div>
            ))}
          </div>
      ...
      <section className="bg-[#141414] py-[100px] px-6 text-white text-center">
          ...
          <div className="max-w-[1200px] mx-auto px-6">
            <div className={`flex flex-col ${block.reverse ? "md:flex-row-reverse" : "md:flex-row"} items-center gap-12 md:gap-[80px]`}>
              <div className="flex-1">
                <h2 className="text-[36px] lg:text-[42px] font-semibold text-[#1A1B1F] leading-[1.2] mb-6">{block.title}</h2>
                <p className="text-[#666666] text-[18px] leading-[1.6] mb-8">{block.desc}</p>
                <ul className="space-y-4 mb-10">
                  {block.bullets.map((text, j) => (
                    <li key={j} className="flex items-start gap-3">
                      <div className="mt-1 flex-shrink-0"><StarIcon /></div>
                      <span className="text-[#1A1B1F] text-[16px] font-medium">{text}</span>
                    </li>
                  ))}
                </ul>
      ...
      <section className="bg-white py-[100px]">
        <div className="max-w-[1200px] mx-auto px-6">
          <div className="text-center mb-12">
            <p className="text-[14px] font-bold text-[#0096ff] mb-4 uppercase">What our users say</p>
            <h2 className="text-[36px] md:text-[42px] font-semibold text-[#1a1b1f] mb-8">Loved by the community.</h2>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-16">
            {[{ num: "562,192", label: "Users" }, { num: "4.9/5", label: "Rating" }, { num: "39", label: "Countries" }, { num: "625,000+", label: "Job applications" }].map((stat, i) => (
              <div key={i} className="bg-[#f8f8f7] p-8 rounded-[12px] text-center border border-[#e5e7eb]/50">
                <div className="text-[32px] font-bold text-[#141414] mb-1">{stat.num}</div>
                <div className="text-[14px] font-bold text-[#666666] uppercase">{stat.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-[#f8f8f7] py-[100px]">
        <div className="max-w-[900px] mx-auto px-6">
          <div className="text-center mb-12">
            <h2 className="text-[36px] md:text-[42px] font-semibold text-[#1a1b1f] leading-[1.2]">Frequently Asked Questions</h2>
          </div>
          <div className="space-y-3">
            {faqItems.map((item, i) => (
              <div key={i} className="overflow-hidden rounded-[10px] border border-[#e1e8ed] bg-white">
                <button onClick={() => setOpenFaq(openFaq === i ? null : i)} className="flex w-full items-center justify-between px-6 py-5 text-left hover:bg-[#f4f7fa] transition-colors">
                ...
                <div className={`overflow-hidden transition-all duration-300 ${openFaq === i ? "max-h-[500px]" : "max-h-0"}`}>
                  <div className="border-t border-[#e1e8ed] px-6 py-5 text-[16px] leading-[1.6] text-[#555]">{item.answer}</div>
                </div>
              </div>
            ))}
          </div>
